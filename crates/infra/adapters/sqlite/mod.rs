...
impl MetadataProvider for SqliteAdapter {
    async fn fetch_metadata(&self, dsn: &str) -> Result<DatabaseMetadata, DbOperationError> {
        ...
        metadata.schemas = Self::parse_schemas();
    ...
    async fn fetch_table_detail(
        ...
        let query = Self::table_detail_query(table);
        let json = self.execute_meta_query(dsn, &query).await?;
        let (columns, indexes, foreign_keys, triggers) = Self::parse_table_detail_combined(&json)?;
        let primary_key = Self::extract_primary_key(&columns);

        Ok(Table {
            schema: Self::MAIN_SCHEMA.to_string(),
    ...
    async fn fetch_table_columns_and_fks(
        ...
        let query = Self::table_columns_and_fks_query(table);
        let json = self.execute_meta_query(dsn, &query).await?;
...
impl QueryExecutor for SqliteAdapter {
    async fn execute_preview(
        ...
        let order_columns = self
            .fetch_preview_order_columns(dsn, table)
            .await
            .unwrap_or_default();
